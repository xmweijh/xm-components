...
组件注册方法在utils中有install.ts 文件，如果你有更好的方式，可以自行编写。添加组件后需要再global.d.ts 添加组件引入，此处是为了让你的组件库添加volar支持，后期在使用组件的时候在tsconfig.json types添加xm-components/global就可以在你的编译器上代码提示
...
<script setup lang="ts">
  // 引用打包的目录
  import { IButton } from '@xm-components/index';
  // 引用packages的目录
...
pnpm link --global xm-components
